...
d_2000_processed <- d_2000 %>% select(ANO_ELEICAO,NUM_TURNO,UF,COD_MUN_IBGE,NOME_MUNICIPIO,SIGLA_PARTIDO,QTDE_VOTOS,DESC_SIT_TOT_TURNO)
...
d_2004_processed <- d_2004 %>% select(ANO_ELEICAO,NUM_TURNO,UF,COD_MUN_IBGE,NOME_MUNICIPIO,SIGLA_PARTIDO,QTDE_VOTOS,DESC_SIT_TOT_TURNO)
...
d_2000 <- d_2000 %>% mutate(key=paste0(COD_MUN_IBGE,SIGLA_PARTIDO))
d_2004 <- d_2004 %>% mutate(key=paste0(COD_MUN_IBGE,SIGLA_PARTIDO))

length(which(d_2000$key==d_2004$key))

d_2004[which(d_2004$key %>% duplicated()),]
d_2004[which(is.na(d_2004$SIGLA_PARTIDO)),]

d_2000[which(d_2000$key %>% duplicated()),]

d_2000 %>% select(COD_MUN_IBGE,SIGLA_PARTIDO) %>% ungroup() %>% count()
d_2000 %>% select(COD_MUN_IBGE,SIGLA_PARTIDO) %>% ungroup() %>% unique() %>% count()

d_2004 %>% select(COD_MUN_IBGE,SIGLA_PARTIDO) %>% ungroup() %>% count()
d_2004 %>% select(COD_MUN_IBGE,SIGLA_PARTIDO) %>% ungroup() %>% unique() %>% count()


for (i in 1:100) {
  print(length(which(d_2000$key[i]==d_2000$key)))
}
